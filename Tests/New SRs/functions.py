...
numberOfMetBins = 5
numberOfMetRegions = 2*numberOfMetBins - 1


CR = ['SR', 'onemuCR', 'twomuCR', 'twoeCR']
CR_label = ['SR', '1muCR', '2muCR', '2eCR']

numberOfRegions = len(CR)*numberOfMetRegions + 1


met_regions = ['ISR' + repr(i+1) for i in range(numberOfMetBins)] + \
                ['ESR' + repr(i+1) for i in range(numberOfMetBins - 1)]
    
regions = [cr + ' - ' + met_region for cr in CR_label for met_region in met_regions] + ['gammajetCR']
...
# all other arguments: string
    ...
    return_array = [0 for _ in range(len(met_regions))]
    ...
    for cr in selected_regions:
        return_array = [ return_array[i] + sample[key][cr][met_regions[i]][tightness][isolation][Na_Nb] for i in range(len(met_regions)) ]
...
def plot_coefficient(coeff, title):
    ...
    for key in coeff:
        ...
        if key == 'gammajets_sig' or key == 'gammajets_bkg':
            plt.errorbar([len(met_regions)+j*0.04], coeff[key]['mean'][len(met_regions)], yerr=coeff[key]['sigma'][len(met_regions)], fmt=color[j], marker='o', linestyle=linestyle, linewidth=2, label='gammajets')
        
        j = j + 1
    
    
    plt.xticks(range(len(met_regions) + 1), met_regions + ["gammajetCR"], fontsize=(7-numberOfMetBins)*10)
    ...
    plt.ylabel(title + '    ', rotation=0, fontsize=45)
...
def purity(Na, Nb, Ma, Mb, c1, c2, c3, R):
    ...
    return_dict = {'mean':     [0 for _ in range(len(Na))], 
                   'sigma':    [0 for _ in range(len(Na))],
                   'sigma_c1': [0 for _ in range(len(Na))],
                   'sigma_c2': [0 for _ in range(len(Na))],
                   'sigma_c3': [0 for _ in range(len(Na))],
                   'sigma_R':  [0 for _ in range(len(Na))]}
...
def analyze_coefficient(coeff):
    ...
    avarage_array['mean'][len(met_regions)]  = avarage_array_gj['mean'][len(met_regions)]
    avarage_array['sigma'][len(met_regions)] = avarage_array_gj['sigma'][len(met_regions)]
    ...
    return {'mean' : [ avarage_array['mean'][i%(M-1)] for i in range(len(met_regions)*4) ] + [ avarage_array['mean'][M-1] ], \
            'sigma': [ avarage_array['sigma'][i%(M-1)] for i in range(len(met_regions)*4) ] + [ avarage_array['sigma'][M-1] ],\
            'systematics': [ sigma_systematics[i%(M-1)] for i in range(len(met_regions)*4) ] + [ sigma_systematics[M-1] ]}
