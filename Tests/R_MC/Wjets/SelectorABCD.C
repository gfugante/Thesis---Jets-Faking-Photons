...
void SelectorABCD::Begin(TTree * /*tree*/)
{
   ...
   minx = -40.;
   maxx = 100.;
   //not_iso_threshold = 40.;

   xbinning = new Double_t[intervallix+1];
   xbinning[0] = 0.;
   xbinning[1] = 1.;
   //xbinning[2] = not_iso_threshold;
   xbinning[2] = 2.;
   ...
   twoECR_H[0] = new TH2F("2eCR - ISR1","2eCR_H - ISR1",intervallix,xbinning,intervalliy,miny,maxy);
   twoECR_H[1] = new TH2F("2eCR - ISR2","2eCR_H - ISR2",intervallix,xbinning,intervalliy,miny,maxy);
   twoECR_H[2] = new TH2F("2eCR - ISR3","2eCR_H - ISR3",intervallix,xbinning,intervalliy,miny,maxy);
   twoECR_H[3] = new TH2F("2eCR - ESR1","2eCR_H - ESR1",intervallix,xbinning,intervalliy,miny,maxy);
   twoECR_H[4] = new TH2F("2eCR - ESR2","2eCR_H - ESR2",intervallix,xbinning,intervalliy,miny,maxy);
...
Bool_t SelectorABCD::Process(Long64_t entry)
{
   ...
   if( *run != 364106 && *run != 364107 && *run != 364108 && *run != 364120 && *run != 364121 && *run != 364122 && *run != 364134 && *run != 364135 && *run != 364136 && *run != 364148 && *run != 364149 && *run != 364150) return kFALSE;
...
void SelectorABCD::FillGraphs(TH2F **g, int i, Float_t met) {
   ...
   //if (ph_ptcone20[i]/ph_pt[i]<0.05) {
      ...
      weight = 1.;
      //weight = luminosity * (*xSec_SUSY * *k_factor * *filter_eff * *evsf_good_nominal_EL * *evsf_good_nominal_MU * *evsf_leading_nominal_PH * *pu_weight * *mconly_weight) / *mc_weight_sum;
      
      if(abs(*mconly_weight)>100.) weight=1.;

      
      if(*xSec_SUSY>500.) weight=0.;//cout << ph_truth_origin[i]  << "\t" << ph_truth_type[i] << "\t" << weight << endl;
