...
void SelectorABCD::Begin(TTree * /*tree*/)
{
   ...
   isolation_tight = new TGraph();
   isolation_tight4 = new TGraph();

   tight_iso = new Int_t[4];
   tight4_iso = new Int_t[4];
...
void SelectorABCD::Terminate()
{
   ...
   TLine *track = new TLine( 0.05, -100., 0.05, 1000.);
   track->SetLineColor(kRed);

   TLine *calo = new TLine(0., 0., 100., 0. );
   calo->SetLineColor(kRed);
 

   c1->cd();

   isolation_tight->SetTitle("SR - ISR1 Tight");
   isolation_tight->GetXaxis()->SetTitle("Track Isolation");
   isolation_tight->GetYaxis()->SetTitle("Calo Isolation");

   isolation_tight->Draw("ap");
   track->Draw("same");
   calo->Draw("same");
   ...
   isolation_tight4->SetTitle("SR - ISR1 Tight-4");
   isolation_tight4->GetXaxis()->SetTitle("Track Isolation");
   isolation_tight4->GetYaxis()->SetTitle("Calo Isolation");

   isolation_tight4->Draw("ap");
   track->Draw("same");
   calo->Draw("same");
...
/*
  ...
  */ 
   ...
   cout << "      Eventi Tight" << endl;
   cout << "__________________________________" << endl << endl;   
   cout << setprecision(4) << "\t" << tight_iso[3] << "\t" << tight_iso[2] << endl;
   cout << "\t" << tight_iso[0] << "\t" << tight_iso[1] << endl;
   cout << "__________________________________" << endl << endl;   


   cout << endl << endl;
   cout << "__________________________________" << endl << endl;
   cout << "      Eventi Tight-4" << endl;
   cout << "__________________________________" << endl << endl;   
   cout << setprecision(4) << "\t" << tight4_iso[3] << "\t" << tight4_iso[2] << endl;
   cout << "\t" << tight4_iso[0] << "\t" << tight4_iso[1] << endl;
...
void SelectorABCD::FillGraphs(TH2F **g, int i, Float_t met) {
   ...
   // colo vs track iso per la SR - ISR1
   if (met > 150000.) {

      if (ph_isTight[i]) {

         isolation_tight->SetPoint(j, ph_ptcone20[i]/ph_pt[i], (ph_topoetcone40[i]/1000.) - (0.022*(ph_pt[i]/1000.)+2.45));
         if(ph_ptcone20[i]/ph_pt[i]<0.05 && (ph_topoetcone40[i]/1000.) - (0.022*(ph_pt[i]/1000.)+2.45)<0.)        tight_iso[0]++;
         if(!(ph_ptcone20[i]/ph_pt[i]<0.05) && (ph_topoetcone40[i]/1000.) - (0.022*(ph_pt[i]/1000.)+2.45)<0.)     tight_iso[1]++;
         if(!(ph_ptcone20[i]/ph_pt[i]<0.05) && !((ph_topoetcone40[i]/1000.) - (0.022*(ph_pt[i]/1000.)+2.45)<0.))  tight_iso[2]++;
         if(ph_ptcone20[i]/ph_pt[i]<0.05 && !((ph_topoetcone40[i]/1000.) - (0.022*(ph_pt[i]/1000.)+2.45)<0.))     tight_iso[3]++;
         j++;
      }

      if (!(ph_isEM[i] & tight4) && !ph_isTight[i] ) {

         isolation_tight4->SetPoint(j4, ph_ptcone20[i]/ph_pt[i], (ph_topoetcone40[i]/1000.) - (0.022*(ph_pt[i]/1000.)+2.45));
         if(ph_ptcone20[i]/ph_pt[i]<0.05 && (ph_topoetcone40[i]/1000.) - (0.022*(ph_pt[i]/1000.)+2.45)<0.)        tight4_iso[0]++;
         if(!(ph_ptcone20[i]/ph_pt[i]<0.05) && (ph_topoetcone40[i]/1000.) - (0.022*(ph_pt[i]/1000.)+2.45)<0.)     tight4_iso[1]++;
         if(!(ph_ptcone20[i]/ph_pt[i]<0.05) && !((ph_topoetcone40[i]/1000.) - (0.022*(ph_pt[i]/1000.)+2.45)<0.))  tight4_iso[2]++;
         if(ph_ptcone20[i]/ph_pt[i]<0.05 && !((ph_topoetcone40[i]/1000.) - (0.022*(ph_pt[i]/1000.)+2.45)<0.))     tight4_iso[3]++;
         j4++;
      }
