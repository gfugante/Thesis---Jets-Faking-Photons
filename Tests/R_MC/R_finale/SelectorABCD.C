...
Bool_t SelectorABCD::Process(Long64_t entry)
{
   ...
   for (int i = 0, n =  ph_pt.GetSize(); i < n; ++i) {
      ...
      //if( ph_truth_type[i] != 15 && ph_truth_type[i] != 16 && ph_truth_type[i] != 17 )    return kFALSE;
...
void SelectorABCD::FillGraphs(TH2F **g, int i, Float_t met) {
   ...
   else if ( Eiso > 3. || (Eiso < 0. && Etrack > track_limit) )   xbin = 1.;
   else return kFALSE; // eventi nella strip
