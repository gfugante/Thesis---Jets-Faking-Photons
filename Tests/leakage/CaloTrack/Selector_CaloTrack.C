...
#include <TTree.h>
#include <cstring>
#define PATH_OUT "dati_Znunugamma.txt"
...
void Selector_CaloTrack::Begin(TTree * /*tree*/)
{
   ...
   track_limit = new Float_t[10];
   iso_limit = 3.;
   iso_max = 140.;
   track_max = 1.;

   file = new TFile("root_files/isolamento2_Znunugamma.root", "RECREATE");
   ...
   histo[0] = new TH1F("type","type",20,0.,20.);
   histo[1] = new TH1F("type","type",20,0.,20.);
   histo[2] = new TH1F("type","type",20,0.,20.);
   histo[3] = new TH1F("type","type",20,0.,20.);
   histo[4] = new TH1F("type","type",20,0.,20.);

   histo2[0] = new TH1F("origin","origin",60,0.,60.);
   histo2[1] = new TH1F("origin","origin",60,0.,60.);
   histo2[2] = new TH1F("origin","origin",60,0.,60.);
   histo2[3] = new TH1F("origin","origin",60,0.,60.);
   histo2[4] = new TH1F("origin","origin",60,0.,60.);

   histo3 = new TH1F("Eiso_gammajetCR","Eiso_gammajetCR",100,-30.,140.);

   for (int i = 0; i < 10; i++)
      track_limit[i] = Float_t(i+1)*0.1;
   ...
   for (int i = 0; i < 10; i++) {
      ...
      SR[i] = new Float_t*[10];
      oneMuCR[i] = new Float_t*[10];
      twoMuCR[i] = new Float_t*[10];
      twoECR[i] = new Float_t*[10];

      for (int j = 0; j < 10; j++) {
         
         SR[i][j] = new Float_t[4];
         oneMuCR[i][j] = new Float_t[4];
         twoMuCR[i][j] = new Float_t[4];
         twoECR[i][j] = new Float_t[4];
                     ...
                     if(fabs(ph_ZVtx[i])<250.) {
                        ...
                        if(*n_jet==0 ||(*n_jet==1 && *n_jet_good==*n_jet)) {
                           ...
                           if(*n_el_baseline==0 && *n_mu_baseline==0) {
                              ...
                              if( ph_truth_type[i] == 13 )  FillGraphs(SR, i, *met_tst_et, 0);
               ...
               if(fabs(ph_met_nomuon_dphi[i]) > 0.4) 
                  ...
                  if(*n_jet==0 ||(*n_jet==1 && *n_jet_good==*n_jet)) 
                     ...
                     if(*n_el_baseline==0 && *n_mu==1 && *n_mu_baseline==1)
                        ...
                        if( ph_truth_type[i] == 13 )  FillGraphs(oneMuCR, i, *met_nomuon_tst_et, 1);
                     ...
                     if(fabs(ph_met_nomuon_dphi[i]) > 0.4) 
                        ...
                        if(*n_jet==0 ||(*n_jet==1 && *n_jet_good==*n_jet)) 
                           ...
                           if(*n_el_baseline==0 && *n_mu==2 && *mumu_m>10000 && *n_mu_baseline==2) 
                              ...
                              if( ph_truth_type[i] == 13 )  FillGraphs(twoMuCR, i, *met_nomuon_tst_et, 2);
                     ...
                     if(fabs(ph_met_noele_dphi[i]) > 0.4) 
                        ...
                        if(*n_jet==0 ||(*n_jet==1 && *n_jet_good==*n_jet)) 
                           ...
                           if(*n_mu_baseline==0 && *n_el==2 && *ee_m>10000 && *n_el_baseline==2) 
                              ...
                              if( ph_truth_type[i] == 13 )  FillGraphs(twoECR, i, *met_noelectron_tst_et, 3);
                     ...
                     if(fabs(ph_met_wmuon_dphi[i])>0.4 && fabs(ph_met_wmuon_dphi[i])<3) 
                        ...
                        if(*n_jet==0 ||(*n_jet==1 && *n_jet_good==*n_jet))
                           ...
                           if(*n_el_baseline==0 && *n_mu_baseline==0) 
                              ...
                              if( ph_truth_type[i] == 13 )  FillGraphs(gammaJetCR, i);
...
void Selector_CaloTrack::Terminate()
{
   ...
   for (int i = 0; i < 4; i++) {
      ...
      for (int j = 0; j < 5; j++) {
         ...
         for (int k = 0; k < 10; k++) {
            out << "    " << setprecision(3) << fixed << track_limit[k] << ":" << endl;
            out << "      " << "Na: " << mat[i][j][k][0] << endl;
            out << "      " << "Nb: " << mat[i][j][k][1] << endl;
            out << "      " << "Ma: " << mat[i][j][k][2] << endl; 
            out << "      " << "Mb: " << mat[i][j][k][3] << endl;
         }
      }
   }
   ...
   for (int k = 0; k < 10; k++) {
      ...
      out << "    " << "Ma: " << gammaJetCR[k][2] << endl; 
      out << "    " << "Mb: " << gammaJetCR[k][3] << endl;
   }
...
void Selector_CaloTrack::FillGraphs(Float_t*** g, int i, Float_t met, int region) {
   ...
   if (met > 150000.) {
      ...
      histo[0]->Fill(ph_truth_type[i], weight);
      histo2[0]->Fill(ph_truth_origin[i], weight);
      ...
      for (int t = 0; t < 10; t++) {
         ...
         // tight non iso
         if ( (Ecalo > iso_limit && Ecalo < iso_max) || ( Ecalo < iso_limit && Etrack > track_limit[t] && Etrack < track_max) ) 
            if ( ph_isTight[i] ) 
               g[0][t][1] += weight;
   ...
   //ISR2
   if(met > 225000.) {
      histo[1]->Fill(ph_truth_type[i], weight);
      histo2[1]->Fill(ph_truth_origin[i], weight);
   ...
   //ISR3
   if(met > 300000.) {
      histo[2]->Fill(ph_truth_type[i], weight);
      histo2[2]->Fill(ph_truth_origin[i], weight);
   ...
   if(met < 225000. && met > 150000.) {
      histo[3]->Fill(ph_truth_type[i], weight);
      histo2[3]->Fill(ph_truth_origin[i], weight);
   ...
   if(met < 300000. && met > 225000.) {
      histo[4]->Fill(ph_truth_type[i], weight);
      histo2[4]->Fill(ph_truth_origin[i], weight);
      for (int t = 0; t < 10; t++) {
