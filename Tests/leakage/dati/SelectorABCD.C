...
void SelectorABCD::Begin(TTree * /*tree*/)
{
   ...
   //file = new TFile("R_Znunu.root", "RECREATE");
/*
   int bin = 20;
   float max = 20.;

   R_mu_H = new TH1F("R_mu","R_mu",bin,0.,max);
   R_el_H = new TH1F("R_el","R_el",bin,0.,max);
   R_jet_H = new TH1F("R_jet","R_jet",bin,0.,max);
*/
                     ...
                     if(fabs(ph_ZVtx[i])<250.) {
                        ...
                        if(*n_jet==0 ||(*n_jet==1 && *n_jet_good==*n_jet)) {
                           ...
                           if(*n_el_baseline==0 && *n_mu_baseline==0) {
                              ...
                              if( ph_truth_type[i] != 13 )     FillGraphs(SR, i, *met_tst_et);
               ...
               if(fabs(ph_met_nomuon_dphi[i]) > 0.4) 
                  ...
                  if(*n_jet==0 ||(*n_jet==1 && *n_jet_good==*n_jet)) 
                     ...
                     if(*n_el_baseline==0 && *n_mu==1 && *n_mu_baseline==1)
                        ...
                        if( ph_truth_type[i] != 13 )   FillGraphs(oneMuCR, i, *met_nomuon_tst_et);
                     ...
                     if(fabs(ph_met_nomuon_dphi[i]) > 0.4) 
                        ...
                        if(*n_jet==0 ||(*n_jet==1 && *n_jet_good==*n_jet)) 
                           ...
                           if(*n_el_baseline==0 && *n_mu==2 && *mumu_m>10000 && *n_mu_baseline==2) 
                              ...
                              if( ph_truth_type[i] != 13 )   FillGraphs(twoMuCR, i, *met_nomuon_tst_et);
                     ...
                     if(fabs(ph_met_noele_dphi[i]) > 0.4) 
                        ...
                        if(*n_jet==0 ||(*n_jet==1 && *n_jet_good==*n_jet)) 
                           ...
                           if(*n_mu_baseline==0 && *n_el==2 && *ee_m>10000 && *n_el_baseline==2) 
                              ...
                              if( ph_truth_type[i] != 13 )   FillGraphs(twoECR, i, *met_noelectron_tst_et);
...
*/
/*
   cout << "R_mu_min: " << R_mu_min << endl;
   cout << "R_el_min: " << R_el_min << endl;
   cout << "R_jet_min: " << R_jet_min << endl;

   R_mu_H->Scale(1./R_mu_H->Integral());
   R_el_H->Scale(1./R_el_H->Integral());
   R_jet_H->Scale(1./R_jet_H->Integral());

   R_mu_H->Draw("hist");
   R_el_H->Draw("histsame");
   R_jet_H->Draw("histsame");
*/
...
void SelectorABCD::FillGraphs(Float_t** g, int i, Float_t pt) {
   ...
   if(pt > 150000.) {
      ...
      R_mu = jet_eta;
/*      R_mu = sqrt( pow(mu_phi[i] - ph_phi[i],2) + pow(mu_eta[i] - ph_eta[i],2) );
      R_el = sqrt( pow(el_phi[i] - ph_phi[i],2) + pow(el_eta[i] - ph_eta[i],2) );
      R_jet = sqrt( pow(jet_phi[i] - ph_phi[i],2) + pow(jet_eta[i] - ph_eta[i],2) );

      if (R_mu < R_mu_min) R_mu_min = R_mu;
      if (R_el < R_el_min) R_el_min = R_mu;
      if (R_jet < R_jet_min) R_jet_min = R_mu;

      R_mu_H->Fill(R_mu, weight);
      R_el_H->Fill(R_el, weight);
      R_jet_H->Fill(R_jet, weight);
*/
...
void SelectorABCD::FillGraphs(Float_t* g, int i) {
