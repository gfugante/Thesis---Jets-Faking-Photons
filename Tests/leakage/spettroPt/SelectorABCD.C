...
#include <fstream>
#include <TTree.h>
#define PATH_OUT "histograms_Wgamma.root"
...
void SelectorABCD::Begin(TTree * /*tree*/)
{
   ...
   gammaJetCR_H->GetXaxis()->SetTitle("p_{t} [GeV]");
   ...
   for(int i = 0; i < 4; i++) {
      ...
      SR_H[i]->GetXaxis()->SetTitle("p_{t} [GeV]");
      oneMuCR_H[i]->GetXaxis()->SetTitle("p_{t} [GeV]");
      twoMuCR_H[i]->GetXaxis()->SetTitle("p_{t} [GeV]");
      twoECR_H[i]->GetXaxis()->SetTitle("p_{t} [GeV]");
...
*/
      ...
      SR_H[i]->GetYaxis()->CenterTitle();
      oneMuCR_H[i]->GetYaxis()->CenterTitle();
      twoMuCR_H[i]->GetYaxis()->CenterTitle();
      twoECR_H[i]->GetYaxis()->CenterTitle();
...
void SelectorABCD::FillGraphs(TH1F **g, int i, Float_t met) {
   ...
   if (met > 150000.) {
      ...
      if (ph_isTight[i])                               g[0] -> Fill(ph_pt[i]/1000., weight);
   ...
   //ISR2
   if(met > 225000.) {
      if (ph_isTight[i])                               g[1] -> Fill(ph_pt[i]/1000., weight);
   ...
   //ISR3
   if(met > 300000.) {
      if (ph_isTight[i] )                              g[2] -> Fill(ph_pt[i]/1000., weight);
   ...
   if(met < 225000. && met > 150000.) {
      if (ph_isTight[i])                               g[3] -> Fill(ph_pt[i]/1000., weight);
   ...
   if(met < 300000. && met > 225000.) {
      if (ph_isTight[i] )                              g[4] -> Fill(ph_pt[i]/1000., weight);
...
void SelectorABCD::FillGraphs(TH1F *g, int i) {
   ...
   if (ph_isTight[i])                               g -> Fill(ph_pt[i]/1000., weight);
